...
#[cyndra_runtime::main(tracing_layer = Logger::init)]
async fn init() -> Result<MyService, cyndra_runtime::Error> {
    Ok(MyService::new())
}
...
impl Logger {
    ...
    pub fn init() -> impl tracing_subscriber::Layer<cyndra_runtime::Registry> {
        let logger = Self {
            sender: LOG_CHANNEL.clone(),
        };
