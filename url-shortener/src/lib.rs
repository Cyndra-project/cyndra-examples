...
async fn build_state(factory: &mut dyn Factory) -> Result<AppState, cyndra_service::Error> {
    ...
    MIGRATOR
        .run(&pool)
        .await
        .map_err(|err| cyndra_service::Error::Database(sqlx::Error::Migrate(Box::new(err))))?;
